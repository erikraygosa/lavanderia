    ...
    <div class="flex flex-wrap items-center justify-between gap-3 mb-6">
        ...
        <div class="flex items-center gap-2">
            ...
            <div x-data="{ copiado: false }" class="relative">
                <button
                    @click="navigator.clipboard.writeText('{{ route('pedidos.ticket.publico', ['id' => $pedido->id, 'token' => config('app.ticket_token')]) }}').then(() => { copiado = true; setTimeout(() => copiado = false, 2000) })"
            ...
            <div class="card">
                ...
                @if($pedido->estado === 'pendiente')
                    <div class="mt-4 space-y-2">
                        ...
                        {{-- Anticipo al recibir el pedido --}}
                        <div class="pt-3 border-t border-gray-100 dark:border-gray-700 space-y-2">
                            <p class="text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wide">Anticipo</p>
                            ...
                            @if(!$mostrarFormAbono)
                                <button wire:click="abrirFormAbono"
                                    ...
                                    💰 {{ $pedido->tieneAnticipo() ? 'Agregar abono' : 'Registrar anticipo' }}
                                </button>
                            @else
                                <div class="bg-amber-50 dark:bg-amber-900/20 border border-amber-200 dark:border-amber-700 rounded-xl p-3 space-y-2">
                                    <p class="text-xs font-semibold text-amber-800 dark:text-amber-300">💰 ¿Cuánto dejó de anticipo?</p>
                                    <div class="relative">
