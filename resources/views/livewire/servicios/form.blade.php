<div>
    <div class="flex items-center gap-3 mb-6">
        <a href="{{ route('servicios.index') }}" class="text-gray-400 hover:text-gray-600">
            <svg class="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 19l-7-7 7-7"/></svg>
        </a>
        <h2 class="text-xl font-semibold text-gray-900">{{ $servicio?->exists ? 'Editar servicio' : 'Nuevo servicio' }}</h2>
    </div>
    ...
    <div class="max-w-xl">
        <div class="card">
            <form wire:submit="guardar" class="space-y-4">
                <div>
                    <label class="label-field">Nombre del servicio *</label>
                    <input wire:model="nombre" type="text" class="input-field" placeholder="Ej. Lavado y planchado" autofocus />
                ...
                <div class="flex items-center gap-3 pt-2 border-t border-gray-100">
                    <button type="submit" class="btn-primary">
                        ...
                        {{ $servicio?->exists ? 'Actualizar' : 'Guardar' }}
                    </button>
